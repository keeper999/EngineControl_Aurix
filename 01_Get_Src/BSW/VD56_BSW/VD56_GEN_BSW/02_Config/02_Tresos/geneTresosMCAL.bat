...
rem Unzip all MCAL plugins
for /F %%i in ('dir /B /S ..\..\..\..\*_Aurix.7z') do (
startenv unzip -y %%i -o%TRESOS_PATH%\%TRESOS_VERSION%\plugins
)
...
echo.
echo ---------------------------------------------------------------------------------------------------
echo Generating .c and .h file from .xdm files in ..\tresos_workspace\%TRESOS_OUTPUT_DIR% folder
echo ---------------------------------------------------------------------------------------------------
echo.
s:\util\choice /c:yn /n Do you want to generate .c and .h file from .xdm files in ..\tresos_workspace\%TRESOS_OUTPUT_DIR% folder [Y]es [N]o ? 
if errorlevel 2 goto END

attrib -R /S /D
set VrTest=
for /f "delims=" %%i in ('type list_xdm_MCAL') do set VrTt=%%i& call :treat
goto CMD

:treat
set VrTest=%VrTest% -g %VrTt%_AurixAS403 %VrTt%.xdm@xdm
goto :eof

:CMD
cd ..\tresos_workspace
call %TRESOS_PATH%\%TRESOS_VERSION%\bin\tresos_cmd.bat -Dwarning=true -DmergeConfigs=true -Dtarget=AURIX -Dderivate=TC277 legacy generate -c %XDM_INPUT_DIR% -o %TRESOS_OUTPUT_DIR% %VrTest% > %TRESOS_OUTPUT_DIR%\%LOG_FILE_NAME%
type %TRESOS_OUTPUT_DIR%\%LOG_FILE_NAME%
rd /S /Q %TRESOS_OUTPUT_DIR%\Demo_Aurix
cd ..\02_Tresos
echo.
echo ---------------------------------------------------------------------------------------------------
