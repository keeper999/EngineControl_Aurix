...
/*                                                                            */
/* !File            : STARTUP_Core2.c                                         */
/* !Description     : Start-up software of Core 2                             */
/*                                                                            */
/* !Reference       :                                                         */
/*                                                                            */
/* Coding language  : C                                                       */
 ...
 * %PID: P2017_BSW:0A111667.A-SRC;4 %
...
/******************************************************************************/
/* !FuncName    : STARTUP_Core2_start                                         */
/* !Description : Startup Function of Core 2                                  */
/*                                                                            */
/* !LastAuthor  : L. Baglin                                                   */
/******************************************************************************/
void STARTUP_Core2_start(void)
{
   uint16 u16LocalWdtPassword =
      IfxScuWdt_getCpuWatchdogPasswordInline(&MODULE_SCU.WDTCPU[2]);
   ...
   __setareg(a0, __SDATA1_2);
   __setareg(a1, __SDATA2_2);
   __setareg(a8, __SDATA3_2);
   __setareg(a9, __SDATA4_2);

   /* Load user stack pointer */
   __setareg(sp, __USTACK2);
   __dsync();

   /* Set the PSW to its reset value in case of a warm start,clear PSW.IS */
   __mtcr(CPU_PSW, STARTUP_CORE2_PSW_DEFAULT);
   __isync() ;

   /* Set the PCXS and PCXO to its reset value in case of a warm start */
   u32LocalPcxi  = __mfcr(CPU_PCXI);
   u32LocalPcxi &= STARTUP_CORE2_PCX_O_S_DEFAULT; /*0xfff00000; */
   __mtcr(CPU_PCXI, u32LocalPcxi);

   /* Setup the context save area linked list. */
   /* First calculate nr of CSAs in this area  */
   s32LocalNnOfCsas = ((sint32 *) __CSA2_END - (sint32 *) __CSA2) >> 4;
   ps32LocalPrevCsa = (sint32 *) __CSA2;
   ...
   __mtcr(CPU_ISP, (uint32)__ISTACK2);

   /* Enable Program Cache */
   IfxCpu_setProgramCache(STARTUP_CORE2_ENA_PCACHE);

   /* Enable Data Cache if Configured */
   IfxCpu_setDataCache(STARTUP_CORE2_ENA_DCACHE);
   ...
   /* Disable of internal Core 2 Watchdog */
   ...
   __non_return_call(MAIN_core2_main);
}
